...
def index(request):
    ...
    # number of books containing "The"
    num_books_the = Book.objects.filter(title__icontains='The').count()

    # number of genres containing "Science"
...
class BookDelete(PermissionRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        try:
            # Delete all BookInstances with this book as the foreign key
            BookInstance.objects.filter(book=self.object).delete()
            self.object.delete()
            return HttpResponseRedirect(self.success_url)
        except Exception as e:
            return HttpResponseRedirect(
